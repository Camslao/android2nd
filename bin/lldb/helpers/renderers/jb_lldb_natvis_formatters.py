...
def _evaluate_interpolated_string(interp_string: TypeVizInterpolatedString, ctx_val, wildcards=None, context=None,
    ...
    for expr in interp_string.expr_list:
        ...
        substring = _eval_display_string_expression(ctx_val, expr_text, expr_opts, context, disable_formatting) or ''
...
def _eval_display_string_expression(ctx, expr, opts, context=None, disable_formatting=False):
    rec_level = get_recursion_level()
    if rec_level >= g_max_recursion_level:
        return ''
    result = eval_expression(ctx, expr, None, context)
