...
class GenericPointerVisDescriptor(AbstractVisDescriptor):
    ...
    def output_summary(self, value_non_synth: lldb.SBValue, stream: Stream):
        ...
        symbol: lldb.SBSymbol = sb_addr.GetSymbol()
        if symbol is not None and symbol.IsValid():
            file_name = ''
            module: lldb.SBModule = sb_addr.GetModule()
            ...
            stream.output("!")
            symbol_name = symbol.GetDisplayName() or symbol.GetName()
            if symbol_name is None:
                symbol_name = '<unknown>'
            stream.output(symbol_name)
            stream.output("}")
